...
void Usage(char* exeName){
  ...
  cout<<"-h, --help \t Show help message and exit"<<endl;
	cout<<"-i, --input \t Input file name containing histogram to be read (.root) (e.g. see data/AugerInclinedSpectrum.root)"<<endl;
	cout<<"-s, --histoname \t Name of histogram with spectrum data to be unfolded (default=RecSpectrum)"<<endl;
	cout<<"=============================="<<endl;
}
...
static const struct option options_tab[] = {
  ...
  { "help", no_argument, 0, 'h' },
	...
	{ "histoname", required_argument, 0, 's' },
...
int main(int argc, char *argv[])
{
  ...
  int option_index = 0;
	...
	std::string histoName= "RecSpectrum";

	while((c = getopt_long(argc, argv, "hi:s:",options_tab, &option_index)) != -1) {
	...
	if(!inputFile || inputFile->IsZombie()){
		cerr<<"ERROR: Failed to read input file "<<inputFileName<<" with rec spectrum!"<<endl;
		return -1;
	}
	TH1D* recSpectrum= (TH1D*)inputFile->Get(histoName.c_str());
	if(!recSpectrum){
		cerr<<"ERROR: Failed to read rec spectrum histogram with name "<<histoName<<" from input file "<<inputFileName<<"!"<<endl;
		return -1;
	}
